...
(function (window, document, $, undefined) {
  ...
  var defaults = {
    ...
    iframe: {
      // Iframe template
      tpl: '<iframe id="fancybox-frame{rnd}" name="fancybox-frame{rnd}" className="fancybox-iframe" allowfullscreen="allowfullscreen" allow="autoplay; fullscreen" src=""></iframe>',
    ...
    video: {
      tpl:
        '<video className="fancybox-video" controls controlsList="nodownload" poster="{{poster}}">' +
        '<source src="{{src}}" type="{{format}}" />' +
        'Sorry, your browser doesn\'t support embedded videos, <Link to="{{src}}">download</Link> and watch with your favorite video player!' +
        "</video>",
    ...
    baseTpl:
      ...
      '<div className="fancybox-infobar"><span data-fancybox-index></span>&nbsp;/&nbsp;<span data-fancybox-count></span></div>' +
      '<div className="fancybox-toolbar">{{buttons}}</div>' +
      '<div className="fancybox-navigation">{{arrows}}</div>' +
      '<div className="fancybox-stage"></div>' +
      '<div className="fancybox-caption"><div className="fancybox-caption__body"></div></div>' +
      "</div>" +
      "</div>",
    ...
    btnTpl: {
      download:
        '<Link download data-fancybox-download className="fancybox-button fancybox-button--download" title="{{DOWNLOAD}}" to="javascript:;">' +
      ...
      // Arrows
      arrowLeft:
        '<button data-fancybox-prev className="fancybox-button fancybox-button--arrow_left" title="{{PREV}}">' +
      ...
      smallBtn:
        '<button type="button" data-fancybox-close className="fancybox-button fancybox-close-small" title="{{CLOSE}}">' +
  ...
  $.extend(FancyBox.prototype, {
    ...
    addContent: function (content) {
      ...
      $.each(items, function (i, item) {
        ...
        if ($.isPlainObject(item)) {
          ...
          obj.src = self.opts.src || opts.src || $item.attr("to");
    ...
    createSlide: function (pos) {
      ...
      if (!self.slides[pos] && self.group[index]) {
        $slide = $('<div className="fancybox-slide"></div>').appendTo(
          self.$refs.stage
        );
    ...
    setImage: function (slide) {
      ...
      slide.$content = $('<div className="fancybox-content"></div>')
    ...
    afterLoad: function (slide) {
      ...
      if (slide.opts.protect && slide.$content && !slide.hasError) {
        ...
        if (slide.type === "image") {
          $('<div className="fancybox-spaceball"></div>').appendTo(
            slide.$content
          );
        }
      }
    ...
    updateControls: function () {
      ...
      if (current.type === "image") {
        ...
        $container
          ...
          .attr("to", current.opts.image.src || current.src)
          .show();
...
(function (document, $) {
  ...
  if (fn) {
    ...
    $.extend(true, $.fancybox.defaults, {
      btnTpl: {
        fullScreen:
          '<button data-fancybox-fullscreen className="fancybox-button fancybox-button--fsenter" title="{{FULL_SCREEN}}">' +
...
(function (document, $) {
  ...
  $.extend(FancyThumbs.prototype, {
    ...
    create: function () {
      ...
      // Build the list
      if (!self.$list) {
        self.$list = $('<div className="' + CLASS + '__list">').appendTo(
          self.$grid
        );
      }
      ...
      $.each(instance.group, function (i, item) {
        ...
        list.push(
          '<Link to="javascript:;" tabindex="0" data-index="' +
            i +
            '"' +
            (src && src.length
              ? ' style="background-image:url(' + src + ')"'
              : 'className="fancybox-thumbs-missing"') +
            "></Link>"
        );
      });
  ...
  $.extend(true, $.fancybox.defaults, {
    ...
    share: {
      ...
      tpl:
        ...
        "<h1>{{SHARE}}</h1>" +
        "<p>" +
        '<Link className="fancybox-share__button fancybox-share__button--fb" to="https://www.facebook.com/sharer/sharer.php?u={{url}}">' +
        '<svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="m287 456v-299c0-21 6-35 35-35h38v-63c-7-1-29-3-55-3-54 0-91 33-91 94v306m143-254h-205v72h196" /></svg>' +
        "<span>Facebook</span>" +
        "</Link>" +
        '<Link className="fancybox-share__button fancybox-share__button--tw" to="https://twitter.com/intent/tweet?url={{url}}&text={{descr}}">' +
        '<svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="m456 133c-14 7-31 11-47 13 17-10 30-27 37-46-15 10-34 16-52 20-61-62-157-7-141 75-68-3-129-35-169-85-22 37-11 86 26 109-13 0-26-4-37-9 0 39 28 72 65 80-12 3-25 4-37 2 10 33 41 57 77 57-42 30-77 38-122 34 170 111 378-32 359-208 16-11 30-25 41-42z" /></svg>' +
        "<span>Twitter</span>" +
        "</Link>" +
        '<Link className="fancybox-share__button fancybox-share__button--pt" to="https://www.pinterest.com/pin/create/button/?url={{url}}&description={{descr}}&media={{media}}">' +
        '<svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="m265 56c-109 0-164 78-164 144 0 39 15 74 47 87 5 2 10 0 12-5l4-19c2-6 1-8-3-13-9-11-15-25-15-45 0-58 43-110 113-110 62 0 96 38 96 88 0 67-30 122-73 122-24 0-42-19-36-44 6-29 20-60 20-81 0-19-10-35-31-35-25 0-44 26-44 60 0 21 7 36 7 36l-30 125c-8 37-1 83 0 87 0 3 4 4 5 2 2-3 32-39 42-75l16-64c8 16 31 29 56 29 74 0 124-67 124-157 0-69-58-132-146-132z" fill="#fff"/></svg>' +
        "<span>Pinterest</span>" +
        "</Link>" +
        "</p>" +
        '<p><input className="fancybox-share__input" type="text" value="{{url_raw}}" onclick="select()" /></p>' +
        "</div>",
    },
  });
      ...
      opts: {
        ...
        afterLoad: function (shareInstance, shareCurrent) {
          ...
          shareCurrent.$content
            ...
            .click(function () {
              window.open(this.to, "Share", "width=550, height=450");
              return false;
            });
        },
        mobile: {
